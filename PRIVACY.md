...
Converse is a Chrome extension that indexes your Claude conversation history locally in your
...
All conversation data is stored exclusively in your browser's IndexedDB database on your
own device. This data never leaves your browser. It is never transmitted to the extension
author, any third party, or any server.

Your Paste to File preferences (enabled/disabled, character threshold) are stored in
`chrome.storage.sync`. This syncs across your signed-in Chrome profiles via Google's
infrastructure — no data is sent to any server controlled by this extension.
...
When used in an Incognito window, Converse stores data in a separate, isolated IndexedDB
instance. This data is automatically cleared by Chrome when the Incognito window closes.
No data from Incognito sessions persists to your regular browsing profile.

To use Converse in Incognito, you must explicitly enable it via
`chrome://extensions` → Converse → **Allow in Incognito**.
...
| `storage` | Used to persist settings (Paste to File preferences) in browser storage |
